...
//
// Anime playback is intentionally restricted to the ani-cli path (AllAnime /
// AllManga direct sources only).
// ==============================================================================
...
fragment animeFields on Media {
  ...
  async function anilistSeasonTitle(baseTitle, seasonNumber = 1) {
    ...
    try {
      ...
      const baseNodeTitle = media.title || {};
      const baseResolved = {
        title: baseNodeTitle.english || baseNodeTitle.romaji || baseTitle,
        romaji: baseNodeTitle.romaji || null,
      };
      if (seasonNumber <= 1) return baseResolved;
  ...
  function aniSkipEpisodeFor(item, seasonNumber, episodeNumber) {
    const parsedEpisode = Number.parseInt(episodeNumber, 10);
    if (!Number.isFinite(parsedEpisode) || parsedEpisode <= 0) return null;

    const parsedSeason = Number.parseInt(seasonNumber, 10);
    if (!Number.isFinite(parsedSeason) || parsedSeason <= 0) return parsedEpisode;

    const linked = normalizeEpisodeLinkage(item, parsedSeason, parsedEpisode);
    const linkedEpisode = Number.parseInt(linked, 10);
    if (!Number.isFinite(linkedEpisode) || linkedEpisode <= 0) return parsedEpisode;
  ...
  async function resolveSource(item, episodeNumber, opts = {}) {
    ...
    if (!result) {
      const fallbackType = translationType === "dub" ? "sub" : "dub";
      if (window.omniLog) {
        window.omniLog("AnimePipeline", `${translationType.toUpperCase()} resolving failed on AllAnime. Re-trying with ${fallbackType.toUpperCase()}...`);
      }
    ...
    if (result) {
      if (window.omniLog) {
        const previewUrl = (result.url || "").substring(0, 80);
        window.omniLog("AnimePipeline", `[OK] Resolved successfully via AllAnime! URL: ${previewUrl}...`);
      }
      return { ...result, provider: "AllManga", kind: "direct" };
    }

    // No third-party anime fallback: Omniverse is ani-cli path only.
